...
frappe.ui.form.on('Expense Claim', {
	...
	refresh(frm) {
	    if (frm.doc.__islocal==1){
	        cur_frm.doc.approval_status = "Draft";
	    }
	    if(cur_frm.doc.workflow_state === "Waiting Finance Head Dept" || cur_frm.doc.workflow_state === "Waiting Finance SPV"){
	    //cur_frm.doc.approval_status = "Approved";
	   // frm.set_df_property("approval_status", "hidden", frm.is_new() ? 0 : 1);
	   // frm.set_df_property("approval_status", "read_only", frm.is_new() ? 0 : 1);
	   //frappe.msgprint("hello")
	    var df=frappe.meta.get_docfield("Expense Claim", "approval_status",frm.doc.name);
        df.hidden=0;
        df.read_only=0;
        //cur_frm.doc.approval_status = "Draft";
        //frm.refresh_fields();
			...
			return frm.call({
				...
				callback: function(r) {
					if(!r.exc) {
						...
						var total_jasa = 0;
					    var total_amount = 0;
					    for (var row_pajak in frm.doc.expenses){
					    	total_jasa += frm.doc.expenses[row_pajak].nilai_jasa || 0
					    	total_amount += frm.doc.expenses[row_pajak].sanctioned_amount
					    }
					    var total_pajak = 0
					    if(total_jasa != 0 && total_amount !=0){
					    	 for (var row_pajak in frm.doc.taxes) {
						      var baris_pajak = frm.doc.taxes[row_pajak];
						      if (baris_pajak.rate != 0) {
						        baris_pajak.tax_amount = (total_jasa * baris_pajak.rate) / 100;
						       	baris_pajak.total = total_amount + baris_pajak.tax_amount
						       	total_pajak += baris_pajak.tax_amount
						      }
						    }
						    cur_frm.doc.total_taxes_and_charges = total_pajak
...
frappe.ui.form.on('Expense Claim Detail', {
	 nilai_jasa: function (frm, cdt, cdn) {
	    ...
	    var total_amount = 0;
	    for (var row_pajak in frm.doc.expenses){
	    	total_jasa += frm.doc.expenses[row_pajak].nilai_jasa
	    	total_amount += frm.doc.expenses[row_pajak].sanctioned_amount
	    }
	    var total_pajak = 0
	    for (var row_pajak in frm.doc.taxes) {
...
cur_frm.cscript.refresh = function(doc) {
	...
	cur_frm.remove_custom_button("Bank Entries","View")
...
frappe.ui.form.on("Expense Claim", {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus===1 && ( (cint(frm.doc.total_amount_reimbursed)-cint(frm.doc.total_sanctioned_amount)-cint(frm.doc.total_advance_amount)<=0) || cint(frm.doc.total_amount_reimbursed == 0) ) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "addons.custom_standard.custom_expense_claim.custom_make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
			...
			callback: function(r) {
				...
				doclist[0].tax_or_non_tax = frm.doc.tax_or_non_tax
...
frappe.ui.form.on("Expense Taxes and Charges", {
	...
	rate: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		var total_jasa = 0
		for(var baris in frm.doc.expenses){
			total_jasa += frm.doc.expenses[baris].nilai_jasa
		}
		if(!child.amount) {
			child.tax_amount = flt(total_jasa) * (flt(child.rate)/100);
		}
