...
frappe.ui.form.on('Branch Depreciation Schedule', {
	make_journal_entry: function(frm,cdt,cdn){
		console.log(locals[cdt][cdn].name)
		frappe.call({
			method: "addons.addons.doctype.gias_asset.gias_asset.make_je_depreciation_by_name",
			args:{
				"name" : locals[cdt][cdn].name
			},
			freeze:true,
			callback: function(r){
				frappe.msgprint("Depreciation Entry is created. Please reload to see if any changes.")
			}
		});
	}
})