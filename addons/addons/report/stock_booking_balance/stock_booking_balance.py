...
def execute(filters=None):
	...
	for row in data:
		ste_check = frappe.db.sql(""" 
			...
			AND std.s_warehouse = "{}"
			AND ste.docstatus = 0
		""".format(row["item_code"],row["warehouse"]),as_dict=1)
		...
		row["book_qty"] = dne_qty + ste_qty
		row["aval_qty"] = row["bal_qty"] - row["book_qty"]
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Booking Qty"), "fieldname": "book_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
		{"label": _("Available Qty"), "fieldname": "aval_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
