...
def coba_benerin_material_request(site):
	...
	list_pull_node = frappe.db.sql("""
		...
		WHERE tmr.name IN
		(
			"MR-SMD-1-23-10-00205",
			"MR-SMD-1-23-10-00206",
			"MR-SMD-1-23-10-00208"
		)
...
def lakukan_pull_node_debug_pusat():
	...
	if str(url) != "erp-pusat.gias.co.id":
		return 
	list_event_producer = frappe.db.sql(""" SELECT name FROM `tabEvent Producer` WHERE name LIKE "%smd%" """)
	for row in list_event_producer:
...
@frappe.whitelist()
def update_item():
	cek = frappe.db.sql(""" UPDATE `tabItem` SET ps_approver = "AHO"
		WHERE name IN ("A-AACP-N-000001",
			"A-AACP-N-000002",
			"A-AACP-N-000003",
			"A-AACP-N-000004",
			"A-AACP-N-000005",
	...
	cek = frappe.db.sql(""" UPDATE `tabItem` SET ps_approver = "ICHA"
		WHERE name IN ("B-TRIM-N-000001",
		"B-TRIM-N-000002",
		"B-TRIM-N-000003",
		"B-TRIM-N-000004",
		"B-TRIM-N-000005",
